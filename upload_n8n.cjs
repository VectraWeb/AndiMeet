...
async function upload() {
    const data = JSON.parse(fs.readFileSync('./REcepcionista.patched.json', 'utf8'));
    const workflowId = 'ABz0WledZziOVMQY'; // Bot REcepcionista ID
    ...
    const response = await fetch(url, {
        ...
        body: JSON.stringify({
            nodes: data.nodes,
            connections: data.connections, settings: { executionOrder: data.settings?.executionOrder || 'v1' },
            name: data.name
        })
    });
